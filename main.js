...
async function callAPI() {
  ...
  */
	...
	// Options, second parameter needed in the fetch because default is GET and we doin' a POST. Usual jazz, content type or else wont understand what the heck you're sending/and specifying the domain name which is what is going to be used to prefix the urls in this project.
	const options = {
		method: "POST",
		headers: {
	...
	try {
		...
		// removed the prefix because this API returns urls with the prefix already.
		const urlLink = `${data.secureShortURL}` // replaced concatenated string value with template literals for ES6 consistency
